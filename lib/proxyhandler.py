...
class ProxyRequestHandler(tornado.web.RequestHandler):
    ...
    def generate_ssl_certificate(hostname):
        ...
        if not os.path.isfile(certpath):
            ...
            cmd2 = ["openssl", "x509", "-req", "-days", "3650", "-CA", options['cacert'], "-CAkey", options['cakey'],
    ...
    async def _my_handle_request(self):
        ...
        if 'throttle_down_peer_logging' in self.options.keys() and len(self.options['throttle_down_peer_logging']) > 0:
            with SqliteDict(plugins.redirector.ProxyPlugin.DynamicWhitelistFile, autocommit=True) as mydict:
                if 'peers' not in mydict.keys():
                    mydict['peers'] = {}

                prev = mydict.get('peers', {})
                peerIP = plugins.redirector.ProxyPlugin.get_peer_ip(self.request)
                elapsed = 0
                ...
                self.logger.dbg('Logging stats for peer {}: elapsed: {}, count: {}'.format(
                    peerIP, elapsed, prev[peerIP]['count']
                ), color='yellow')

        self.logger.info('[REQUEST] {} {}'.format(self.request.method, self.request.uri),
        ...
        class MyResponse(http.client.HTTPResponse):
            def __init__(self, req, origreq):
                ...
                if req != None:
                    ...
                    if hasattr(origreq, 'reason'):
                        self.reason = origreq.reason
                    else:
                        self.reason = ''

                    if hasattr(origreq, 'headers'):
                        self.headers = origreq.headers.copy()
                    else:
                        self.headers = {}
        ...
        if not dont_fetch_response:
            try:
                ...
                if plugins.IProxyPlugin.proxy2_metadata_headers[
                    'ignore_response_decompression_errors'] in self.request.headers.keys():
                ...
                if (outbound_origin != '' and outbound_origin == inbound_origin) and \
                    ...
                    self.logger.dbg(
                        f'About to catch reverse-proxy loop detection: outbound_origin: {outbound_origin}, ip: {ip}, server_address: {self.server_address}')
                ...
                if plugins.IProxyPlugin.proxy2_metadata_headers['domain_front_host_header'] in new_req_headers.keys():
                    ...
                    new_req_headers['Host'] = self.request.headers[
                        plugins.IProxyPlugin.proxy2_metadata_headers['domain_front_host_header']]
        ...
        if (not ignore_response_decompression_errors) and ('Accept-Encoding' in self.request.headers.keys()):
            ...
            if override_resp_enc in res.headers.keys():
                ...
                if content_encoding not in encs:
                    ...
                    if not reencoded:
                        self.logger.err(
                            "Server returned response encoded in {} but client expected one of: {} - we couldn't handle any of these. The client will receive INCORRECTLY formatted response!".format(
        ...
        self.logger.info(
            '[RESPONSE] HTTP {} {}, length: {}, keep-alive: {}'.format(res.status, res.reason, len(res_body), ka),
        ...
        try:
            ...
            if options['debug']:
                if originChanged:
                    del self.request.headers['Host']
                    if plugins.IProxyPlugin.proxy2_metadata_headers[
                        'override_host_header'] in self.request.headers.keys():
                        outbound_origin = self.request.headers[
                            plugins.IProxyPlugin.proxy2_metadata_headers['override_host_header']]
